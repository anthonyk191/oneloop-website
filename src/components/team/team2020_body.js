import './team2020_body.css';

import React, { useEffect } from 'react';

const Team2020_body = () =>{
	return(
		<React.Fragment>
			<div className="oneloop-team2020-body">
				<div className="oneloop-team2019-title">
					<h1>2020-2021 Team</h1>
				</div>
				<h2>Executive Commitee</h2>
				<div className="oneloop-team2019 executive team-mobile-1">
					<div className="executive-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Louk_Goldberg.jpg"/>
							<h4>LOUK GOLDBERG<br/>
							<span>President</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20192020/Annanya_Tyagi.jpeg"/>
							<h4>ANNANYA TYAGI<br/>
							<span>Project Manager</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Sam_Menigoz.png"/>
							<h4>SAM MENIGOZ<br/>
							<span>Chief Systems<br/>Engineer</span></h4>
						</div>
					</div>
				</div>

				<h2>Brakes</h2>
				<div className="oneloop-team2019 brakes team-mobile">
					<div className="brakes-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Jordan_Hall.jpg"/>
							<h4>JORDAN HALL<br/>
							<span>Brakes Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Timothy_Groshev.jpeg"/>
							<h4>TIMOTHY GROSHEV<br/>
							<span>Brakes Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Gaurav_Dhirar.jpg"/>
							<h4>GAURAV DHIRAR<br/>
							<span>Brakes Member</span></h4>
						</div>
					</div>
					<div className="brakes-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Preseton_Vanderpan.jpeg"/>
							<h4>PRESTON<br/>VANDERPAN<br/>
							<span>Brakes Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20192020/Ashir+Bhalla-Levine.jpeg"/>
							<h4>ASHIR<br/>BHALLA-LEVINE<br/>
							<span>Brakes Member</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 brakes team-web">
					<div className="brakes-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Jordan_Hall.jpg"/>
							<h4>JORDAN HALL<br/>
							<span>Brakes Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Timothy_Groshev.jpeg"/>
							<h4>TIMOTHY GROSHEV<br/>
							<span>Brakes Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Gaurav_Dhirar.jpg"/>
							<h4>GAURAV DHIRAR<br/>
							<span>Brakes Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Preseton_Vanderpan.jpeg"/>
							<h4>PRESTON<br/>VANDERPAN<br/>
							<span>Brakes Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20192020/Ashir+Bhalla-Levine.jpeg"/>
							<h4>ASHIR<br/>BHALLA-LEVINE<br/>
							<span>Brakes Member</span></h4>
						</div>
					</div>
				</div>

				<h2>Business</h2>
				<div className="oneloop-team2019 business team-mobile">
					<div className="business-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Anthony_Nguyen.jpg"/>
							<h4>ANTHONY NGUYEN<br/>
							<span>Business Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Yashvi_Deokule.jpg"/>
							<h4>YASHVI DEOKULE<br/>
							<span>Business Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Pratham_Goradia.jpeg"/>
							<h4>PRATHAM GORADIA<br/>
							<span>Business Co-Lead</span></h4>
						</div>
					</div>
					<div className="business-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Omri_Steinberg-Tatman.png"/>
							<h4>OMRI<br/>STEINBERG-TATMAN<br/>
							<span>Business Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Zbynka_Kekula.jpg"/>
							<h4>ZBYNKA KEKULA<br/>
							<span>Business Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Lucas_Dahlke.jpg"/>
							<h4>LUCAS DAHLKE<br/>
							<span>Business Member</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 business team-web">
					<div className="business-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Anthony_Nguyen.jpg"/>
							<h4>ANTHONY NGUYEN<br/>
							<span>Business Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Yashvi_Deokule.jpg"/>
							<h4>YASHVI DEOKULE<br/>
							<span>Business Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Pratham_Goradia.jpeg"/>
							<h4>PRATHAM GORADIA<br/>
							<span>Business Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Omri_Steinberg-Tatman.png"/>
							<h4>OMRI<br/>STEINBERG-TATMAN<br/>
							<span>Business Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Zbynka_Kekula.jpg"/>
							<h4>ZBYNKA KEKULA<br/>
							<span>Business Member</span></h4>
						</div>
					</div>
					<div className="business-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Lucas_Dahlke.jpg"/>
							<h4>LUCAS DAHLKE<br/>
							<span>Business Member</span></h4>
						</div>
					</div>
				</div>

		

				<h2>Controls</h2>
				<div className="oneloop-team2019 controls team-mobile">
					<div className="controls-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Rithik_Sachdeva.jpeg"/>
							<h4>RITHIK SACHDEVA<br/>
							<span>Controls Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Jayden_Patel.jpg"/>
							<h4>JAYDEN PATEL<br/>
							<span>Controls Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Matthew_Pranata.jpg"/>
							<h4>MATTHEW<br/>PRANATA<br/>
							<span>Controls Member</span></h4>
						</div>
					</div>
					<div className="controls-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Janak_Khadka.jpeg"/>
							<h4>JANAK KHADKA<br/>
							<span>Controls Member</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 controls team-web">
					<div className="controls-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Rithik_Sachdeva.jpeg"/>
							<h4>RITHIK SACHDEVA<br/>
							<span>Controls Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Jayden_Patel.jpg"/>
							<h4>JAYDEN PATEL<br/>
							<span>Controls Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Matthew_Pranata.jpg"/>
							<h4>MATTHEW<br/>PRANATA<br/>
							<span>Controls Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Janak_Khadka.jpeg"/>
							<h4>JANAK KHADKA<br/>
							<span>Controls Member</span></h4>
						</div>
					</div>
				</div>


				<h2>Power</h2>
				<div className="oneloop-team2019 power team-web">
					<div className="power-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Ryan_Parker.jpg"/>
							<h4>RYAN PARKER<br/>
							<span>Power Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Forrest_Fankhauser.jpg"/>
							<h4>FORREST FANKHAUSER<br/>
							<span>Power Lead</span></h4	>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Akam_Sihota.jpeg"/>
							<h4>AKAM SIHOTA<br/>
							<span>Power Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Matthew_Benson.jpeg"/>
							<h4>MATTHEW BENSON<br/>
							<span>Power Member</span></h4>
						</div>
					</div>
				</div>
				<div className="oneloop-team2019 power team-mobile">
					<div className="power-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Ryan_Parker.jpg"/>
							<h4>RYAN PARKER<br/>
							<span>Power Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Forrest_Fankhauser.jpg"/>
							<h4>FORREST FANKHAUSER<br/>
							<span>Power Lead</span></h4	>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Akam_Sihota.jpeg"/>
							<h4>AKAM SIHOTA<br/>
							<span>Power Member</span></h4>
						</div>
					</div>
					<div className="power-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Matthew_Benson.jpeg"/>
							<h4>MATTHEW BENSON<br/>
							<span>Power Member</span></h4>
						</div>
					</div>
				</div>

				<h2>Propulsion</h2>
				<div className="oneloop-team2019 propulsion team-mobile">
					<div className="propulsion-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Aaron_Bass.jpg"/>
							<h4>AARON BASS<br/>
							<span>Chief Propulsion <br/>Engineer</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Jason_Ciola.jpg"/>
							<h4>JASON CIOLA<br/>
							<span>Propulsion Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Michael_Cox.jpg"/>
							<h4>MICHAEL COX<br/>
							<span>Propulsion Co-Lead</span></h4>
						</div>
					</div>
					<div className="propulsion-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Isabella_Elliot.jpg"/>
							<h4>ISABELLA ELLIOT<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Mihir_Kulkarni.jpg"/>
							<h4>MIHIR KULKARNI<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Jessica_Nicolas.jpg"/>
							<h4>JESSICA NICOLAS<br/>
							<span>Propulsion Member</span></h4>
						</div>
					</div>
					<div className="propulsion-row row row-3">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Macayla_Thomas.jpg"/>
							<h4>MACAYLA THOMAS<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Isabel_Baeza.jpg"/>
							<h4>ISABEL BAEZA<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Pedy_Louie_Pascual.jpg"/>
							<h4>PEDY LOUIE<br/>PASCUAL<br/>
							<span>Propulsion Member</span></h4>
						</div>
					</div>
					<div className="propulsion-row row row-4">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Joseph_Michael_Tiu.jpg"/>
							<h4>JOSEPH TIU<br/>
							<span>Propulsion Member</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 propulsion team-web">
					<div className="propulsion-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Aaron_Bass.jpg"/>
							<h4>AARON BASS<br/>
							<span>Chief Propulsion <br/>Engineer</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Jason_Ciola.jpg"/>
							<h4>JASON CIOLA<br/>
							<span>Propulsion Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Michael_Cox.jpg"/>
							<h4>MICHAEL COX<br/>
							<span>Propulsion Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Isabella_Elliot.jpg"/>
							<h4>ISABELLA ELLIOT<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Mihir_Kulkarni.jpg"/>
							<h4>MIHIR KULKARNI<br/>
							<span>Propulsion Member</span></h4>
						</div>
					</div>
					<div className="propulsion-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Jessica_Nicolas.jpg"/>
							<h4>JESSICA NICOLAS<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Macayla_Thomas.jpg"/>
							<h4>MACAYLA THOMAS<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Isabel_Baeza.jpg"/>
							<h4>ISABEL BAEZA<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Pedy_Louie_Pascual.jpg"/>
							<h4>PEDY LOUIE<br/>PASCUAL<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Joseph_Michael_Tiu.jpg"/>
							<h4>JOSEPH TIU<br/>
							<span>Propulsion Member</span></h4>
						</div>
					</div>
				</div>

				<h2>Stability</h2>
				<div className="oneloop-team2019 stability team-mobile">
					<div className="stability-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Mark_Hagiwara.jpeg"/>
							<h4>MARK HAGIWARA<br/>
							<span>Stability Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Mark_Hagiwara.jpeg"/>
							<h4>ANJALI GUPTA<br/>
							<span>Stability Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Mark_Hagiwara.jpeg"/>
							<h4>KANSAS SEUNG<br/>
							<span>Stability Co-Lead</span></h4>
						</div>
					</div>
					<div className="stability-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Kyle_Heinzman.jpeg"/>
							<h4>KYLE HEINZMAN<br/>
							<span>Stability Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Kyle_Heinzman.jpeg"/>
							<h4>ANDREW GUNLOCK<br/>
							<span>Stability Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Kyle_Heinzman.jpeg"/>
							<h4>WEIJIN TANG<br/>
							<span>Stability Member</span></h4>
						</div>
					</div>
					<div className="stability-row row row-3">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Kyle_Heinzman.jpeg"/>
							<h4>ZOE WILF<br/>
							<span>Stability Member</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 stability team-web">
					<div className="stability-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Mark_Hagiwara.jpeg"/>
							<h4>MARK HAGIWARA<br/>
							<span>Stability Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Anjali_Gupta.jpeg"/>
							<h4>ANJALI GUPTA<br/>
							<span>Stability Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Kansas_Seung.jpeg"/>
							<h4>KANSAS SEUNG<br/>
							<span>Stability Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Kyle_Heinzman.jpeg"/>
							<h4>KYLE HEINZMAN<br/>
							<span>Stability Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Andrew_Gunlock.jpeg"/>
							<h4>ANDREW GUNLOCK<br/>
							<span>Stability Member</span></h4>
						</div>
					</div>
					<div className="stability-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Weijin_Tang.jpeg"/>
							<h4>WEIJIN TANG<br/>
							<span>Stability Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Zoe_Wilf.jpeg"/>
							<h4>ZOE WILF<br/>
							<span>Stability Member</span></h4>
						</div>
					</div>
				</div>

				<h2>Structures</h2>
				<div className="oneloop-team2019 structures team-mobile">
					<div className="structures-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Pooja_Rao.jpg"/>
							<h4>POOJA RAO<br/>
							<span>Structures Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Riley_Chan.jpg"/>
							<h4>RILEY CHAN<br/>
							<span>Structures Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Gabriel_Gebru.jpg"/>
							<h4>GABRIEL GEBRU<br/>
							<span>Structures Member</span></h4>
						</div>
					</div>
					<div className="structures-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Zi_Rui_Kerk.jpg"/>
							<h4>ZI RUI KERK<br/>
							<span>Structures Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Michelle_Zhang.jpg"/>
							<h4>MICHELLE ZHANG<br/>
							<span>Structures Member</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 structures team-web">
					<div className="structures-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Pooja_Rao.jpg"/>
							<h4>POOJA RAO<br/>
							<span>Structures Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Riley_Chan.jpg"/>
							<h4>RILEY CHAN<br/>
							<span>Structures Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Gabriel_Gebru.jpg"/>
							<h4>GABRIEL GEBRU<br/>
							<span>Structures Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Zi_Rui_Kerk.jpg"/>
							<h4>ZI RUI KERK<br/>
							<span>Structures Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Michelle_Zhang.jpg"/>
							<h4>MICHELLE ZHANG<br/>
							<span>Structures Member</span></h4>
						</div>
					</div>
				</div>

				<h2>Structures / Pressure</h2>
				<div className="oneloop-team2019 pressure team-mobile-1">
					<div className="pressure-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Mursal_Wardak.jpg"/>
							<h4>MURSAL WARDAK<br/>
							<span>Structures / Pressure<br/>Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Alexander_Weinstein.jpeg"/>
							<h4>ALEXANDER<br/>WEINSTEIN<br/>
							<span>Structures / Pressure<br/>Member</span></h4>
						</div>
					</div>
				</div>

				<div className="team2019-buttons">
					<div className="team2019-to-team2018">
						<a href="/team2019">Previous Year</a>
					</div>
					<div className="team2019-to-team2020">
						<a href="/team2021">Next Year</a>
					</div>
				</div>
			</div>
		</React.Fragment>
	);
}

export default Team2020_body;
