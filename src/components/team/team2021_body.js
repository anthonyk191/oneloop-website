import './team2020_body.css';

import React, { useEffect } from 'react';

const Team2020_body = () =>{
	return(
		<React.Fragment>
			<div className="oneloop-team2020-body">
				<div className="oneloop-team2019-title">
					<h1>2020-2021 Team</h1>
				</div>

				<h2>Executive Team</h2>
				<div className="oneloop-team2019 controls team-mobile">
					<div className="controls-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Jayden_Patel.jpeg"/>
							<h4>JAYDEN PATEL<br/>
							<span>President</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Omri_Steinberg-Tatman.jpg"/>
							<h4>OMRI STEINBERG TATMAN<br/>
							<span>Project Manager</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Jordan_Hall.jpg"/>
							<h4>JORDAN<br/>HALL<br/>
							<span>Chief Engineer</span></h4>
						</div>
					</div>
					<div className="controls-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Alex_Shaban.jpeg"/>
							<h4>ALEX SHABAN<br/>
							<span>Chief Financial<br/>Officer</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 controls team-web">
					<div className="controls-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Jayden_Patel.jpeg"/>
							<h4>JAYDEN PATEL<br/>
							<span>President</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Omri_Steinberg-Tatman.jpg"/>
							<h4>OMRI STEINBERG TATMAN<br/>
							<span>Project Manager</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Jodan_Hal.jpg"/>
							<h4>JORDAN<br/>HALL<br/>
							<span>Chief Engineer</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Alex_Shaban.jpeg"/>
							<h4>ALEX SHABAN<br/>
							<span>Chief Financial<br/>Officer</span></h4>
						</div>
					</div>
				</div>

				<h2>Senior Engineers</h2>
				<div className="oneloop-team2019 executive team-mobile-1">
					<div className="executive-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Louk_Goldberg.jpg"/>
							<h4>LOUK GOLDBERG<br/>
							<span></span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Samual_Menigoz.jpeg"/>
							<h4>SAMUAL MENIGOZ<br/>
							<span>Project Manager</span></h4>
						</div>
					</div>
				</div>

				<h2>Brakes</h2>
				<div className="oneloop-team2019 executive team-mobile-1">
					<div className="executive-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Ashir_Bhalla-Levine.jpg"/>
							<h4>ASHIR<br/>BHALLA-LEVINE<br/>
							<span>Brakes Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Gaurav_Dhirar.jpeg"/>
							<h4>GAURAV DHIRAR<br/>
							<span>Brakes Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Henry_Lin.png"/>
							<h4>HENRY LIN<br/>
							<span>Brakes Member</span></h4>
						</div>
					</div>
				</div>

				<h2>Business</h2>
				<div className="oneloop-team2019 stability team-mobile">
					<div className="stability-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Pratham_Goradia.jpeg"/>
							<h4>PRATHAM GORADIA<br/>
							<span>Business Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Zbynka_Kekula.jpeg"/>
							<h4>ZBYNKA KEKULA<br/>
							<span>Business Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Andy_Yen.jpeg"/>
							<h4>ANDY YEN<br/>
							<span>Business Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20202021/Lucas_Dahlke.jpg"/>
							<h4>LUCAS DAHLKE<br/>
							<span>Business Member</span></h4>
						</div>
					</div>
					<div className="stability-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Jinguang_Chai.jpeg"/>
							<h4>JINGUANG CHAI<br/>
							<span>Business Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Lucas_Dahlke.jpeg"/>
							<h4>LUCAS DAHLKE<br/>
							<span>Business Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Rebekah_Friedman.jpeg"/>
							<h4>REBEKAH FRIEDMAN<br/>
							<span>Business Member</span></h4>
						</div>
					</div>
					<div className="stability-row row row-3">
					<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Ruri_Sakakibara.jpeg"/>
							<h4>RURI SAKAKIBARA<br/>
							<span>Business Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Sofia_Stahl.jpeg"/>
							<h4>SOFIA STAHL<br/>
							<span>Business Memberr</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 stability team-web">
					<div className="stability-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Pratham_Goradia.jpeg"/>
							<h4>PRATHAM GORADIA<br/>
							<span>Business Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Zbynka_Kekula.jpeg"/>
							<h4>ZBYNKA KEKULA<br/>
							<span>Business Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Andy_Yen.jpeg"/>
							<h4>ANDY YEN<br/>
							<span>Business Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Jinguang_Chai.jpeg"/>
							<h4>JINGUANG CHAI<br/>
							<span>Business Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Lucas_Dahlke.jpeg"/>
							<h4>LUCAS DAHLKE<br/>
							<span>Business Member</span></h4>
						</div>
					</div>
					<div className="stability-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Revekah_Friedman.jpeg"/>
							<h4>REBEKAH FRIEDMAN<br/>
							<span>Business Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Ruri_Sakakibara.jpeg"/>
							<h4>RURI SAKAKIBARA<br/>
							<span>Business Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Sofia_Stahl.jpeg"/>
							<h4>SOFIA STAHL<br/>
							<span>Business Member</span></h4>
						</div>
					</div>
				</div>



				<h2>Controls</h2>
				<div className="oneloop-team2019 controls team-mobile">
					<div className="controls-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Jayden_Patel.jpeg"/>
							<h4>JAYDEN PATEL<br/>
							<span>Controls Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Rithik_Sachdeva.jpg"/>
							<h4>RITHIK SACHDEVA<br/>
							<span>Controls Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Andrew_Gunlock.jpg"/>
							<h4>ANDREW<br/>GUNLOCK<br/>
							<span>Controls Member</span></h4>
						</div>
					</div>
					<div className="controls-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Ananya_Tyagi.jpeg"/>
							<h4>ANANYA TYAGI<br/>
							<span>Controls Member</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 controls team-web">
					<div className="controls-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Jayden_Patel.jpeg"/>
							<h4>JAYDEN PATEL<br/>
							<span>Controls Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Rithik_Sachdeva.jpg"/>
							<h4>RITHIK SACHDEVA<br/>
							<span>Controls Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Andrew_Gunlock.jpg"/>
							<h4>ANDREW<br/>GUNLOCK<br/>
							<span>Controls Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20192020/Annanya_Tyagi.jpeg"/>
							<h4>ANNANYA TYAGI<br/>
							<span>Controls Member</span></h4>
						</div>
					</div>
				</div>


				<h2>Power</h2>
				<div className="oneloop-team2019 stability team-mobile">
					<div className="stability-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Forrest_Fankhausner.jpeg"/>
							<h4>FORREST FANKHAUSER<br/>
							<span>Power Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Omri_Steinberg-Tatman.jpeg"/>
							<h4>OMRI STEINBERG-TATMAN<br/>
							<span>Power Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Bella_Doherty.jpeg"/>
							<h4>BELLA DOHERTY<br/>
							<span>Power Member</span></h4>
						</div>
					</div>
					<div className="stability-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Connor_Lewis.jpeg"/>
							<h4>CONNOR LEWIS<br/>
							<span>Power Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Christian_Molina.jpeg"/>
							<h4>CHRISTIAN MOLINA<br/>
							<span>Power Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Jan_Truong.jpeg"/>
							<h4>JAN TRUONG<br/>
							<span>Power Member</span></h4>
						</div>
					</div>
					<div className="stability-row row row-3">
					<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Megan_Zhang.jpeg"/>
							<h4>MEGAN ZHANG<br/>
							<span>Power Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Parth_Pawar.jpeg"/>
							<h4>PARTH PAWAR<br/>
							<span>Power Memberr</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Shyam_Agarwal.jpeg"/>
							<h4>SHYAM AGARWAL<br/>
							<span>Power Member</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 stability team-web">
					<div className="stability-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Forrest_Fankhausner.jpeg"/>
							<h4>FORREST FANKHAUSER<br/>
							<span>Power Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Omri_Steinberg-Tatman.jpeg"/>
							<h4>OMRI STEINBERG-TATMAN<br/>
							<span>Power Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Bella_Doherty.jpeg"/>
							<h4>BELLA DOHERTY<br/>
							<span>Power Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Connor_Lewis.jpeg"/>
							<h4>CONNOR LEWIS<br/>
							<span>Power Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Christian_Molina.jpeg"/>
							<h4>CHRISTIAN MOLINA<br/>
							<span>Power Member</span></h4>
						</div>
					</div>
					<div className="stability-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Jan_Truong.jpeg"/>
							<h4>JAN TRUONG<br/>
							<span>Power Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Megan_Zhang.jpeg"/>
							<h4>MEGAN ZHANG<br/>
							<span>Power Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Parth_Pawar.jpeg"/>
							<h4>PARTH PAWAR<br/>
							<span>Power Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Shyam_Agarwal.jpeg"/>
							<h4>SHYAM AGARWAL<br/>
							<span>Power Member</span></h4>
						</div>
					</div>
				</div>

				<h2>Propulsion</h2>
				<div className="oneloop-team2019 stability team-mobile">
					<div className="stability-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Isabel_Baeza.jpeg"/>
							<h4>ISABEL BAEZA<br/>
							<span>Propulsion Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Joseph_Tiu.jpeg"/>
							<h4>JOSEPH TIU<br/>
							<span>Propulsion Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Ali_Gangeh.jpeg"/>
							<h4>ALI GANGEH<br/>
							<span>Propulsion Member</span></h4>
						</div>
					</div>
					<div className="stability-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Harry_Yang.jpeg"/>
							<h4>HARRY YANG<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Lucas_Dahlke.jpeg"/>
							<h4>LUCAS DAHLKE<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Marco_Orta.jpeg"/>
							<h4>MARCO ORTA<br/>
							<span>Propulsion Member</span></h4>
						</div>
					</div>
					<div className="stability-row row row-3">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Mihir_Juneja.jpeg"/>
							<h4>MIHIR JUNEJA<br/>
							<span>Propulsion Member</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 stability team-web">
					<div className="stability-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Isabel_Baeza.jpeg"/>
							<h4>ISABEL BAEZA<br/>
							<span>Propulsion Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Joseph_Tiu.jpeg"/>
							<h4>JOSEPH TIU<br/>
							<span>Propulsion Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Ali_Gangeh.jpeg"/>
							<h4>ALI GANGEH<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Harry_Yang.jpeg"/>
							<h4>HARRY YANG<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Lucas_Dahlke.jpeg"/>
							<h4>LUCAS DAHLKE<br/>
							<span>Propulsion Member</span></h4>
						</div>
					</div>
					<div className="stability-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Marco_Orta.jpeg"/>
							<h4>MARCO ORTA<br/>
							<span>Propulsion Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Mihir_Juneja.jpeg"/>
							<h4>MIHIR JUNEJA<br/>
							<span>Propulsion Member</span></h4>
						</div>
					</div>
				</div>


				<h2>Stability</h2>
				<div className="oneloop-team2019 brakes team-mobile">
					<div className="brakes-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Anjali_Gupta.jpg"/>
							<h4>ANJALI GUPTA<br/>
							<span>Stability Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Mark_Hagiwara.jpeg"/>
							<h4>MARK HAGIWARA<br/>
							<span>Stability Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Patrick_Hong.jpg"/>
							<h4>PATRICK HONG<br/>
							<span>Stability Member</span></h4>
						</div>
					</div>
					<div className="brakes-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Weijing_Tang.jpeg"/>
							<h4>WEIJIN TANG<br/>
							<span>Stability Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Zoe_Wilf.jpeg"/>
							<h4>ZOE WILF<br/>
							<span>Stability Member</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 brakes team-web">
					<div className="brakes-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Anjali_Gupta.jpg"/>
							<h4>ANJALI GUPTA<br/>
							<span>Stability Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Mark_Hagiwara.jpeg"/>
							<h4>MARK HAGIWARA<br/>
							<span>Stability Co-Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Patrick_Hong.jpg"/>
							<h4>PATRICK HONG<br/>
							<span>Stability Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Weijin_Tang.jpeg"/>
							<h4>WEIJIN TANG<br/>
							<span>Stability Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Zoe_Wilf.jpeg"/>
							<h4>ZOE WILF<br/>
							<span>Stability Member</span></h4>
						</div>
					</div>
				</div>

				<h2>Structures</h2>
				<div className="oneloop-team2019 brakes team-mobile">
					<div className="brakes-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Rajasibi_Rajendran.jpg"/>
							<h4>RAJASIBI RAJENDRAN<br/>
							<span>Structures Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Anusheh_Abdullah.jpeg"/>
							<h4>ANUSHEH ABDULLAH<br/>
							<span>Structures Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Aryan_Mondkar.jpg"/>
							<h4>ARYAN MONDKAR<br/>
							<span>Structures Member</span></h4>
						</div>
					</div>
					<div className="brakes-row row row-2">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Bryan_Tiang.jpeg"/>
							<h4>BRYAN TIANG<br/>
							<span>Structures Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Joey_Voss.jpeg"/>
							<h4>JOEY VOSS<br/>
							<span>Structures Member</span></h4>
						</div>
					</div>
				</div>

				<div className="oneloop-team2019 brakes team-web">
					<div className="brakes-row row row-1">
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Rajasibi_Rajendran.jpg"/>
							<h4>RAJASIBI RAJENDRAN<br/>
							<span>Structures Lead</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Anusheh_Abdullah.jpeg"/>
							<h4>ANUSHEH ABDULLAH<br/>
							<span>Structures Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Aryan_Mondkar.jpg"/>
							<h4>ARYAN MONDKAR<br/>
							<span>Structures Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Bryan_Tiang.jpeg"/>
							<h4>BRYAN TIANG<br/>
							<span>Structures Member</span></h4>
						</div>
						<div className="team-member">
							<img className="team-member" src="/images/team/20212022/Joey_Voss.jpeg"/>
							<h4>JOEY VOSS<br/>
							<span>Structures Member</span></h4>
						</div>
					</div>
				</div>

				<div className="team2019-buttons">
					<div className="team2019-to-team2018">
						<a href="/team2019">Previous Year</a>
					</div>
					<div className="team2019-to-team2018">
						<a href="/team2021">Next Year</a>
					</div>
				</div>
			</div>
		</React.Fragment>
	);
}

export default Team2020_body;
